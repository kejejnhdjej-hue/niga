...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'ban') {
      ...
      await interaction.reply({ embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('User Banned').addFields(
    ...
    if (interaction.commandName === 'kick') {
      ...
      await interaction.reply({ embeds: [new EmbedBuilder().setColor(0xFF6600).setTitle('User Kicked').addFields(
        { name: 'User', value: `${target.user.tag}`, inline: true },
        { name: 'Moderator', value: `${interaction.user.tag}`, inline: true },
        { name: 'Reason', value: reason }
      )]});
    ...
    if (interaction.commandName === 'mute') {
      ...
      await interaction.reply({ embeds: [new EmbedBuilder().setColor(0xFFFF00).setTitle('User Muted').addFields(
        { name: 'User', value: `${target.user.tag}`, inline: true },
        { name: 'Moderator', value: `${interaction.user.tag}`, inline: true },
  ...
  if (interaction.isButton() && interaction.customId === 'create_ticket') {
    ...
    await ticketChannel.send({
      content: `<@${user.id}>`,
      embeds: [new EmbedBuilder().setColor(0x5865F2).setTitle('Ticket Opened')
